...
summary(dataset$PENNSize10)

#CORRELATIONS,
dataset.numerical <- dataset[,c(4,5,8,9,10,11)]

correlations <- cor(dataset.numerical, use = "pairwise.complete.obs")
xtable(round(correlations, 2))

#STANDARD DEVIATIONS
sd(dataset$PENNSize10)
...
#WEIGHTED SUM BY THE NUMBER OF AVAILABLE COMPONENTS
weighted.SUM <- dataset$SUM/number.of.nonna.variables

xtable(summary(lm(weighted.SUM~ Year, data = dataset)))
...
rdis.SUM <- RDestimate(weighted.SUM ~ Year, data = dataset, cutpoint = 2009)
summary(rdis.SUM)
