...
rdis.index <- RDestimate(weighted.index ~ Year, data = total.data, cutpoint = 2009)
...
#USING GGPLOT2
require(ggplot2)
ggplot(total.data, aes(Year, weighted.index,  color = "D")) +
...
##### INFORMALITY ####
plot(x = informality$year, y = informality$yi_yf)
abline(lm(informality$yi_yf~informality$year), col ="red")
summary(lm(total.data$weighted.index~total.data$Year))
xtable(summary(lm(total.data$weighted.index~total.data$Year)))
...
RDestimate(yi_yf ~ year , informality.full, cutpoint = 2009)
